...
#import "HostingTableViewController.h"
...
static NSManagedObjectContext *context1;
static NSPersistentContainer *container1;

#define storyboard [UIStoryboard storyboardWithName:@"Main" bundle:nil]


@interface HostingTableViewController ()
{
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title=@"Hosting";
    createF=[createFunc sharedCreateFunc];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    eventArray=[[createF getAllEvents]mutableCopy];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        container1= appdel.persistentContainer;
        
        context1=container1.viewContext;

        [context1 deleteObject:[eventArray objectAtIndex:indexPath.row]];
